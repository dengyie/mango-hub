...
const LoadChart = ({ uuid, data = [], embedded = false }: LoadChartProps) => {
  ...
  useEffect(() => {
    if (!embedded && avaliableView.length > 0) {
      setHoursView(avaliableView[0].label);
    }
  }, [max_record_preserve_time, embedded]);

  const colors = ["#F38181", "#FCE38A", "#EAFFD0", "#95E1D3"];
  const primaryColor = colors[0];
  const secondaryColor = colors[1];
  const cn = embedded
    ? "w-full max-w-full min-w-0 flex flex-col h-full gap-4"
    : "w-full max-w-full md:max-w-72 md:min-w-72 flex flex-col h-full gap-4";
  const chartMargin = {
    top: 0,
    right: 16,
    bottom: 0,
  ...
  const timeFormatter = (value: any, index: number) => {
    if (index === 0 || index === chartData.length - 1) {
      if (
        presetViews[0].label === activeHoursView ||
        activeHoursView === "real-time" ||
        activeHoursView === t("common.real_time")
      ) {
  ...
  const isRealtime =
    activeHoursView === t("common.real_time") || activeHoursView === "real-time";
  ...
  const chartData = isRealtime
    ? realtimeData
    : activeHoursView === presetViews[0].label
    ? fillMissingTimePoints(remoteData ?? [], minute, hour * 4, minute * 2)
    : (() => {
        const selectedHours =
          presetViews.find((v) => v.label === activeHoursView)?.hours ||
          avaliableView.find((v) => v.label === activeHoursView)?.hours ||
          24;
    ...
    <div className="flex flex-col items-center gap-4 w-full max-w-screen">
      ...
      <div
        className={`gap-4 grid w-full justify-items-center mx-auto max-w-full${
          embedded ? " grid-cols-2 max-sm:grid-cols-1" : ""
        }`}
        style={
          embedded
            ? undefined
